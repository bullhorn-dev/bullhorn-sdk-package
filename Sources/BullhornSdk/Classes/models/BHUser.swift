...
struct BHUser: Codable, Hashable {
    ...
    func belongsToCategory(_ categoryId: Int) -> Bool {
        return categories?.contains(where: { $0.id == categoryId }) ?? false
    }
    
    func belongsToChannel(_ channelId: String) -> Bool {
    ...
    func attributedBio(isActive: Bool = false, baseColor: UIColor = UIColor.secondary()) -> NSAttributedString {
        guard let validText = bio else { return NSAttributedString() }

...
public class BHID3Parser: NSObject {

	public static func isGoodForStream(_ url: URL, handler: @escaping (Bool, Bool, Bool) -> Void) {
		let parser = BHID3Parser(url, quickTest: true)
	...
	var isID3: Bool? {
		didSet {
			if isQuick {
                ...
                self.quickHandler?(self.isID3 ?? false, isGoodForStream, self.isVideo)
	...
	private var quickHandler: ((Bool, Bool, Bool) -> Void)?

	func quickTest(_ handler: @escaping (Bool, Bool, Bool) -> Void) {
...
extension BHID3Parser {
    ...
    fileprivate func containsVideo(_ mimeType: String) -> Bool {
        guard let uti = UTType(mimeType: mimeType) else { return false }
...
extension BHID3Parser: URLSessionDataDelegate {
	...
	public func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
		...
		if let httpResponse = response as? HTTPURLResponse {
            ...
            if let mimeType = httpResponse.mimeType {
                isVideo = self.containsVideo(mimeType)
            }
		}
	...
	public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
		DispatchQueue.main.async {
			...
			if self.isQuick && self.inParsing && !self.dataReceived && self.isID3 == nil {
				//we had no data
				self.quickHandler?(false, false, false)
				self.quickHandler = nil
			}
		}
	}

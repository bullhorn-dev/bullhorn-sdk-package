...
open class BaseAttributedTextView: UIView {
    ...
    open var onTimestampTouchUpInside: ((BaseAttributedTextView, Any) -> Void)?
    ...
    open var highlightedLinkRects: [CGRect]? {
    ...
    private func calculateLinksRangeRectsIfNeeded() {
        guard let text = attributedText?.string/*, _linkFramesCache == nil*/ else {
            return
        }
    ...
    open var highlightedTimestampValue: Any? {
        if let range = _highlightedTimestampRange, let val = attributedText?.attributedSubstring(from: range) {
            return val.string
        } else {
            return nil
        }
    ...
    open var highlightedTimestampRange: Range<String.Index>? {
        if let str = attributedText?.string, let range = _highlightedTimestampRange {
            return Range(range, in: str)
        } else {
            return nil
        }
    ...
    open var highlightedTimestampRects: [CGRect]? {
        if let range = highlightedTimestampRange {
            return rects(for: range)
        } else {
            return nil
        }
    ...
    open var highlightedTimestampAttributes: [NSAttributedString.Key: Any]? {
        didSet {
            setNeedsDisplayText(changedGeometry: false)
        }
    }

    open var disabledTimestampAttributes: [NSAttributedString.Key: Any]? {
        didSet {
            setNeedsDisplayText(changedGeometry: false)
        }
    }

    // MARK: - Timestamp rect calculation

    private var _timestampFramesCache: [RangeRects]?

    func invalidateTimestampFramesCache() {
        _timestampFramesCache = nil
    }

    private func calculateTimestampsRangeRectsIfNeeded() {
        ...
        for match in matches {
            let rects = _backend.enclosingRects(forGlyphRange: match.range)
            newTimestampFramesCache.append(RangeRects(range: match.range, rects: rects))
        }

        _timestampFramesCache = newTimestampFramesCache
    }

    private func timestampRange(at point: CGPoint) -> NSRange? {
        calculateTimestampsRangeRectsIfNeeded()

        let textOrigin = _backend.textOrigin
        let adjustedPoint = CGPoint(x: point.x - textOrigin.x, y: point.y - textOrigin.y)

        guard let timestampFramesCache = _timestampFramesCache else {
            return nil
        }

        for rr in timestampFramesCache {
            for rect in rr.rects {
                if rect.contains(adjustedPoint) {
                    return rr.range
                }
    ...
    override open func layoutSubviews() {
        ...
        if bounds.size != prevSize {
            ...
            invalidateTimestampFramesCache()
        }
    }
    ...
    private class TrackingControl: UIControl {
        ...
        override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
            if super.point(inside: point, with: event) {
                if let parent = parent, (parent.linkRange(at: convert(point, to: parent._backend.view)) != nil || parent.timestampRange(at: convert(point, to: parent._backend.view)) != nil) {
                    return true
                }
            }
            return false
    ...
    private var _trackedTimestampRange: NSRange?

    private var _linkHighlightView: (UIView & LinkHighlightViewProtocol)?
    private var _timestampHighlightView: (UIView & LinkHighlightViewProtocol)?
    ...
    private var _highlightedTimestampRange: NSRange? {
        didSet {
            if oldValue != _highlightedTimestampRange {
                _timestampHighlightView?.removeFrom(textView: self)
                _timestampHighlightView = nil
                if let range = _highlightedTimestampRange,
                   let cache = _timestampFramesCache,
                   let factory = linkHighlightViewFactory
                {
                    for item in cache {
                        if item.range == range {
                            ...
                            let highlightView = factory.createView(
                                enclosingRects: item.rects.map {
                                    CGRect(x: $0.origin.x - unionRect.minX,
                                           y: $0.origin.y - unionRect.minY,
                                           width: $0.width,
                            ...
                            insertSubview(highlightView, belowSubview: _backend.view)

                            let textOrigin = _backend.textOrigin
                            highlightView.frame = convert(
                                unionRect.offsetBy(dx: textOrigin.x, dy: textOrigin.y),
                                from: _backend.view
                            )
                            highlightView.didAdd(to: self)
                            _timestampHighlightView = highlightView
                        }
                    }
                }
                setNeedsDisplayText(changedGeometry: false)
    ...
    func _beginTracking(_ touch: UITouch, with _: UIEvent?) {
        ...
        _trackedTimestampRange = timestampRange(at: _trackingControl.convert(pt, to: _backend.view))
        _highlightedTimestampRange = _trackedTimestampRange
    }
    ...
    func _continueTracking(_ touch: UITouch, with _: UIEvent?) {
        ...
        if let currentDetection = timestampRange(at: _trackingControl.convert(pt, to: _backend.view)) {
            if currentDetection == _trackedTimestampRange {
                _highlightedTimestampRange = _trackedTimestampRange
            } else {
                _highlightedTimestampRange = nil
            }
        } else {
            _highlightedTimestampRange = nil
        }

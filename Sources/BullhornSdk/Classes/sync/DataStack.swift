...
@objc enum DataStackStoreType: Int {
...
@objc class DataStack: NSObject {
    ...
    /**
     ...
     instead.
     */
    @objc lazy var mainContext: NSManagedObjectContext = {
    ...
    @objc private(set) lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
     ...
     */
    @objc init(modelName: String, storeType: DataStackStoreType) {
        self.modelName = modelName
    ...
    /**
     ...
     based and doesn't save to disk, while the second one creates a .sqlite file and stores things there.
     */
    @objc init(modelName: String, bundle: Bundle, storeType: DataStackStoreType) {
        self.modelName = modelName
    ...
    /**
     ...
     change that.
     */
    @objc init(modelName: String, bundle: Bundle, storeType: DataStackStoreType, storeName: String) {
        self.modelName = modelName
        self.modelBundle = bundle
        self.storeType = storeType
     ...
     */
    @objc init(modelName: String, bundle: Bundle, storeType: DataStackStoreType, storeName: String, containerURL: URL) {
        self.modelName = modelName
        self.modelBundle = bundle
        self.storeType = storeType
        self.storeName = storeName
    ...
    /**
     ...
     based and doesn't save to disk, while the second one creates a .sqlite file and stores things there.
     */
    @objc init(model: NSManagedObjectModel, storeType: DataStackStoreType) {
        self.model = model
        self.storeType = storeType
     ...
     */
    @objc func newDisposableMainContext() -> NSManagedObjectContext {
        let context = NSManagedObjectContext(concurrencyType: .mainQueueConcurrencyType)
     ...
     */
    @objc func newNonMergingBackgroundContext() -> NSManagedObjectContext {
        let context = NSManagedObjectContext(concurrencyType: DataStack.backgroundConcurrencyType())
        context.persistentStoreCoordinator = self.persistentStoreCoordinator
        context.undoManager = nil
        context.mergePolicy = NSMergeByPropertyStoreTrumpMergePolicy
     ...
     */
    @objc func performInNewBackgroundContext(_ operation: @escaping (_ backgroundContext: NSManagedObjectContext) -> Void) {
    ...
    /**
     ...
     - parameter operation: The block that contains the created background context.
     */
    @objc func performBackgroundTask(operation: @escaping (_ backgroundContext: NSManagedObjectContext) -> Void) {
    ...
    // Drops the database.
    @objc func drop(completion: ((_ error: NSError?) -> Void)? = nil) {
    ...
    @objc func execute(_ request: NSPersistentStoreRequest, with context: NSManagedObjectContext) throws -> Any {
...
extension FileManager {
    ...
    static var sqliteDirectoryURL: URL {
        #if os(tvOS)
